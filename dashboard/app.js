...
(() => {
  ...
  function 기준그리기(자료, 진짜) {
    ...
    // 파는 쪽을 따로 걸어 뒀으면 그 말을 반드시 한다. 목록은 사는 쪽만
    // 보여 주므로, 이 줄이 없으면 화면에 적힌 전략의 파는 규칙으로 팔고
    // 있다고 읽힌다. 실제로는 다른 규칙으로 팔고 있는데도 그렇다.
    const 매도전략 = String(자료.매도전략 || "");
    const 매도줄 = 전략표.find((ㄱ) => ㄱ.키 === 매도전략);
    const 매도알림 = 매도전략
      ? '<div class="알림">파는 쪽은 <b>' + 안전(매도줄 ? 매도줄.이름 : 매도전략)
        + '</b>로 따로 걸려 있습니다. 위 목록에서 고른 것은 <b>사는 쪽</b>입니다. '
        + '보유 기간 상한도 파는 쪽 것을 씁니다.'
        + (매도줄 ? ' 파는 규칙: ' + 안전(매도줄.한줄) : "")
        + '</div>'
      : "";
    ...
    $("전략알림").innerHTML = (못읽음
      ...
      : (읽기만
        ? '<div class="곁말">지금 걸려 있는 전략입니다. 여기서는 <b>보기만</b> 됩니다. '
          + (매도전략
            ? '이 전략은 <b>사는 쪽</b>만 맡고 있습니다. '
            : '이 전략이 사는 것과 파는 것을 다 맡고 있습니다. ')
