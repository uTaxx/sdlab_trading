...
(() => {
  ...
  function 기준그리기(자료, 진짜) {
    ...
    // 쓰는 전략은 설정 시트가 아니라 서버 DB에 들어 있다. 창구가 그것을
    // 못 읽으므로, 고를 수는 있어도 저장은 안 된다. 고르고 저장했는데
    // 아무 일도 안 일어나는 것이 제일 나쁘므로 미리 말한다.
    const 전략못함 = Array.isArray(자료.아직없는칸) && 자료.아직없는칸.indexOf("전략") >= 0;
    $("전략알림").innerHTML = 전략못함
      ? '<div class="알림">지금 어떤 전략이 걸려 있는지는 이 화면에서 볼 수 없습니다. '
        + '전략은 설정 시트가 아니라 서버 안에 저장되어 있고, 아직 화면으로 내보내지 '
        + '않았습니다. 아래에서 골라 저장해도 전략만은 바뀌지 않습니다.</div>'
      : "";
