...
def _낙폭말(낙폭: float) -> str:
    ...
    if 낙폭 > -10:
        return "이 정도면 견디기 어려운 폭은 아닙니다."
    if 낙폭 > -20:
        return "적지 않은 폭입니다."
    if 낙폭 > -30:
        return "계좌가 한때 20% 넘게 줄어 있었다는 뜻입니다. 대부분 여기서 흔들립니다."
    return (
        "계좌가 한때 30% 넘게 줄어 있었다는 뜻입니다. 이 구간을 못 견디면 "
        "뒤에 붙는 수익률은 받아 보지 못합니다."
    )
...
def 평가글(성적: 기간성적, 정의: 기간정의) -> str:
    ...
    **지어내지 않는다.** 숫자에서 바로 나오는 말만 적는다. 왜 그랬는지는
    ...
    if ㅁ.num_trades == 0:
        return (
            "이 구간에 한 주도 안 샀습니다. 지킨 것이 아니라 아무 일도 안 한 "
            "것이라, 수익률 0%를 좋은 성적으로 읽으면 안 됩니다."
        )

    말: list[str] = []
    if 성적.믿을만한가:
        말.append(f"거래 {ㅁ.num_trades}건, {정의.이름} 동안 {ㅁ.total_return_pct:+.2f}%입니다.")
    else:
        말.append(
            f"{정의.이름} 동안 {ㅁ.total_return_pct:+.2f}%인데 거래가 "
            f"{ㅁ.num_trades}건뿐입니다. 한두 종목이 결과를 통째로 만든 숫자라 "
            "이것으로 전략을 판단하면 안 됩니다."
        )
    ...
    if 최악:
        ...
        단위, 이가, 은는 = ("해", "가", "는") if 정의.쪼갬 == "해" else ("달", "이", "은")
        if 최악[1] >= 0:
            말.append(
                f"쪼개 보면 마이너스인 {단위}{이가} 없습니다"
                f"(제일 나빴던 {단위}{이가} {최악[0]} {최악[1]:+.2f}%)."
            )
        else:
            말.append(f"제일 나빴던 {단위}{은는} {최악[0]}, {최악[1]:+.2f}%입니다.")

    말.append(f"고점에서 제일 많이 빠졌을 때가 {ㅁ.max_drawdown_pct:.2f}%입니다. "
              + _낙폭말(ㅁ.max_drawdown_pct))

    if ㅁ.exposure_pct < 40:
        말.append(
            f"이 구간의 {ㅁ.exposure_pct:.0f}%만 종목을 들고 지냈습니다. "
            "덜 맞은 만큼 덜 벌기도 합니다."
        )
    elif ㅁ.exposure_pct > 90:
        말.append(
            f"이 구간의 {ㅁ.exposure_pct:.0f}%를 종목을 들고 지냈습니다. "
            "거의 내내 시장에 나가 있었다는 뜻입니다."
        )
...
def 비교총평(정의: 기간정의, 줄들, 지금키: str = "") -> str:
    ...
    **1등을 고르라는 말은 절대 안 적는다.** 한 구간에서 제일 좋았던 것을
    ...
    말 = [
        f"{정의.이름} 동안 산 전략 {len(차례)}개 중 플러스는 {len(플러스)}개입니다."
    ]
    if len(플러스) <= len(차례) // 4:
        말.append("대부분이 마이너스라 전략 문제가 아니라 구간 문제로 보입니다.")

    if 안산것:
        말.append(f"한 주도 안 산 전략이 {안산것}개 있어 순위에서 뺐습니다.")
    ...
    if 지금키:
        ...
        if 자리:
            ㅅ = dict(차례)[지금키]
            말.append(
                f"지금 걸린 것은 {len(차례)}개 중 {자리}위, "
                f"{ㅅ.metrics.total_return_pct:+.2f}%입니다."
            )

    얕은것 = min(차례, key=lambda ㄱ: -ㄱ[1].metrics.max_drawdown_pct)
    말.append(
        f"제일 덜 빠진 것은 낙폭 {얕은것[1].metrics.max_drawdown_pct:.2f}%입니다."
    )
    말.append(
        "이 순위에서 1등을 그대로 고르면 안 됩니다. 한 구간에서 제일 좋았던 것을 "
        "고르는 일이 곧 과최적화입니다."
    )
    return " ".join(말)
...
def 전략신호(성적들: dict, 순위: dict | None = None) -> list[신호]:
    ...
    **아무 이유도 못 찾으면 빈 목록을 돌려준다.** 억지로 한 줄 만들면
    ...
    if 긴것 and 긴것.metrics.num_trades >= 20 and 긴것.수익률 <= 0:
        난것.append(신호(
            "확인필요", "5년을 통째로 재도 플러스가 아닙니다",
            f"5년 {긴것.수익률:+.2f}%입니다. 최근 구간이 나쁜 것이 아니라 "
            "이 전략을 걸어 둘 근거가 지금 자료로는 안 나온다는 뜻입니다.",
        ))

    # ── 신호가 아예 안 나오는 경우 ──────────────────────────────
    if 석달 and 석달.metrics.num_trades == 0:
        난것.append(신호(
            "확인필요", "최근 3개월에 한 주도 안 샀습니다",
            "전략이 사라고 한 적이 없습니다. 조건이 너무 좁거나, 매매 대상 "
            "종목 목록이 전략과 안 맞는 것일 수 있습니다.",
        ))
    elif 석달 and 석달.metrics.exposure_pct < 10:
        난것.append(신호(
            "살펴볼것", "최근 3개월에 거의 종목을 안 들고 있었습니다",
            f"이 구간의 {석달.metrics.exposure_pct:.0f}%만 들고 지냈습니다. "
            "손실을 덜 보기도 하지만 벌 기회도 거의 안 잡았다는 뜻입니다.",
        ))
    ...
    if 긴것 and 한해 and 긴것.metrics.max_drawdown_pct < -1:
        ...
        if 몫 >= 0.9:
            난것.append(신호(
                "살펴볼것", "계좌가 제일 많이 줄었던 구간이 최근 1년 안에 있습니다",
                f"최근 1년 사이에 계좌가 제일 많았을 때보다 "
                f"{abs(한해.metrics.max_drawdown_pct):.1f}%까지 줄어든 적이 "
                f"있습니다. 5년을 통틀어 제일 많이 줄었던 것이 "
    ...
    for 이름 in ("3개월", "12개월"):
        ...
        if 버틴쪽 >= 1 / 3 and 하위인가:
            난것.append(신호(
                "살펴볼것", f"{이름}에 다른 전략은 버텼는데 이것만 하위권입니다",
                f"{전체}개 중 {플러스}개가 플러스인데 지금 걸린 것은 {순}위입니다. "
                "장이 나빠서가 아니라 이 전략이 이 구간과 안 맞았다는 쪽입니다.",
            ))
        elif 버틴쪽 < 1 / 4 and 하위인가:
            난것.append(신호(
                "이상없음", f"{이름}이 나쁘지만 다 같이 나빴습니다",
                f"{전체}개 중 플러스가 {플러스}개뿐입니다. 지금 걸린 것은 {순}위인데, "
                "전략 문제가 아니라 구간 문제로 보입니다.",
            ))
    return 난것
...
def 신호글(난것: list[신호]) -> tuple[str, str]:
    ...
    if not 난것:
        return "이상없음", (
            "지금 걸린 전략을 다시 볼 이유는 안 나왔습니다. 이건 잘 벌고 "
            "있다는 뜻이 아니라, 예상 밖으로 굴러가고 있지는 않다는 뜻입니다."
        )
    등급 = "이상없음"
    줄들 = []
    ...
    줄들.append("")
    줄들.append(
        "지금 당장 할 일은 없습니다. 이 신호는 바꾸라는 말이 아니라 한 번 "
        "보라는 말입니다. 바꿀지를 따질 때는 다른 전략도 같은 방법으로 다시 "
        "재 봐야 합니다."
    )
    return 등급, "\n".join(줄들)


def 알림글(등급: str, 신호: str, 전략글: str, 성적들: dict) -> str:
    """폰으로 보낼 글.

    **화면과 달리 앞뒤에 아무 맥락이 없다.** 화면에서는 이 글 바로 아래에
    표와 평가 문장이 같이 있어서 무슨 전략의 무슨 숫자인지가 보인다. 폰에는
    이 글 하나만 간다. 그래서 어느 전략인지와 벌고는 있는지를 글 안에 같이
    넣는다. 안 넣으면 낙폭 숫자 두 개만 남는데, 그것만 보고는 무슨 일이
    난 것인지도 무엇을 해야 하는지도 알 수 없다.
    """
    줄들 = [f"🔎 전략 점검 · {등급}", "", f"지금 걸린 전략은 {전략글}입니다."]

    # 나쁜 소식만 보내면 다 망한 줄 안다. 번 것도 같이 적는다.
    성적글 = [
        f"{이름} {성적들[이름].수익률:+.2f}%"
    ...
    if 성적글:
        줄들.append("돌려 본 성적은 " + ", ".join(성적글) + "입니다.")
