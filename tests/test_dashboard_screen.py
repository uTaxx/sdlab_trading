...
def test_표의_빈줄_칸수가_머리_칸수와_같다():
    ...
    assert 'colspan="9" class="빔">아직 골라서 돌려 본 적이' in 글
...
def test_전략을_골라도_매매는_안_바뀐다고_화면이_말한다():
    """이 탭에서 고른 전략은 과거 시세로 돌려 볼 뿐이다. 그 말이 없으면
    여기서 고른 것이 내일 아침에 실제로 걸린다고 읽는다."""
    assert "실제 매매가 바뀌지도 않습니다" in 글
    assert "전략을 바꾸는 것은 돈이 나가는 결정이라" in 쪽
