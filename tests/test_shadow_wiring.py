...
def test_화면과_파이썬이_같은_짝을_고른다():
    ...
    파이썬것 = 견주기([
        StrategyShadowRow(
            ...
            골랐나=ㄱ["골랐나"], 뒤수익률=ㄱ["뒤수익률"], 지난날수=ㄱ["지난날수"],
            상태=ㄱ["상태"],
        )
    ...
    def 요점(ㄱ):
        return (str(ㄱ["제안일"] if isinstance(ㄱ, dict) else ㄱ.제안일),
                ㄱ["구간"] if isinstance(ㄱ, dict) else ㄱ.구간,
                ㄱ["후보전략"] if isinstance(ㄱ, dict) else ㄱ.후보전략,
                round(ㄱ["차이"] if isinstance(ㄱ, dict) else ㄱ.차이, 6),
                bool(ㄱ["버튼있었나"] if isinstance(ㄱ, dict) else ㄱ.버튼있었나))
