...
# ── 1·2·3 순위 버튼 (2026-09-06) ──────────────────────────────────
#
# 주인이 "1, 2, 3 추천 중 선택"으로 정했다. 전에는 버튼에 구간(1주·1개월·
# 3개월)만 적혀서, 셋이 서로 다른 전략을 가리킬 때 무엇을 먼저 볼지가
# 버튼에 안 나왔다.


def test_순위를_매기면_버튼에_적히고_순서대로_선다():
    from datetime import date

    from muwon.notify.telegram_buttons import 전략버튼, 전략키보드

    판 = 전략키보드([
        전략버튼(키="c", 이름="다", 구간="3개월", 수익률=1.0, 순위=3),
        전략버튼(키="a", 이름="가", 구간="1주", 수익률=9.0, 순위=1),
        전략버튼(키="b", 이름="나", 구간="1개월", 수익률=5.0, 순위=2),
    ], date(2026, 9, 6))

    글들 = [줄[0]["text"] for 줄 in 판["inline_keyboard"]]
    assert "1위 가" in 글들[0]
    assert "2위 나" in 글들[1]
    assert "3위 다" in 글들[2]
    # 수익률도 같이 보여야 무엇을 고르는지 안다.
    assert "+9.0%" in 글들[0]


def test_순위를_안_매기면_예전처럼_구간을_적는다():
    """옛 호출 자리를 그대로 두기 위해서다."""
    from datetime import date

    from muwon.notify.telegram_buttons import 전략버튼, 전략키보드

    판 = 전략키보드([전략버튼(키="a", 이름="가", 구간="1주")], date(2026, 9, 6))

    assert "(1주)" in 판["inline_keyboard"][0][0]["text"]
    assert "위 " not in 판["inline_keyboard"][0][0]["text"]


def test_순위_버튼도_눌린_표시가_붙는다():
    """`고른것표시()`가 앞의 두 글자만 갈아 끼운다. 순위는 글 안에 있어야
    다시 그릴 때 안 지워진다."""
    from datetime import date

    from muwon.notify.telegram_buttons import 고른것표시, 전략버튼, 전략키보드

    판 = 전략키보드([
        전략버튼(키="a", 이름="가", 구간="1주", 수익률=9.0, 순위=1),
        전략버튼(키="b", 이름="나", 구간="1주", 수익률=5.0, 순위=2),
    ], date(2026, 9, 6))
    바뀐것 = 고른것표시(판, "b")

    글들 = [줄[0]["text"] for 줄 in 바뀐것["inline_keyboard"]]
    assert 글들[0].startswith("🔁 1위 ")
    assert 글들[1].startswith("✅ 2위 ")
