...
기준들: tuple[기준, ...] = (
    ...
    기준(
        ...
        "**끄면 손절이 안 걸린다.** 주가가 절반으로 떨어져도 아무 일도 일어나지 않는다. "
    ...
    기준(
        ...
        "한 종목에 자금의 몇 %까지",
        "한 종목의 주가가 절반으로 떨어졌을 때 계좌가 입는 손실이 이 값으로 정해진다. "
        "15%면 계좌 손실이 -7.5%다. 동시보유 수와 곱해서 100%를 넘으면 "
        "현금이 모자라 뒤쪽 종목은 못 산다",
    ...
    기준(
        "min_holding_days", "최소 보유기간(일)", "정수", "0",
        "이 기간 안에는 손절 말고 팔지 않습니다. 0이면 안 겁니다",
        "**손절은 이 기간을 무시하고 언제든 판다.** 최소 보유기간은 너무 일찍 "
        "이익을 확정하지 말라는 뜻이지 손실을 끝까지 안고 가라는 뜻이 아니다. "
        "막는 것은 익절과 트레일링과 전략의 매도 신호 셋이다. 보유 상한보다 크게 "
        "적으면 상한이 이긴다",
        정책필드="min_holding_days", 최소=0, 최대=60,
    ),
    기준(
        "max_entry_slip_pct", "최대 진입 갭(%)", "퍼센트", "5",
        "승인할 때 본 값보다 이만큼 넘게 비싸면 안 삽니다. 0이면 안 막습니다",
        "09:05 매수는 어제 종가로 시장가 주문을 낸다. 그 사이에 밤이 하나 있어서, "
        "갭이 크게 뜬 날에는 승인할 때 본 값과 전혀 다른 값에 산다. 손절선이 -5%인데 "
        "+5%에 사면 사자마자 손절 근처다. **비싼 쪽만 막는다.** 싸게 시작한 날은 "
        "그대로 사고, **파는 것은 절대 안 막는다**. 갭 하락한 날 손절을 막으면 이 값이 "
        "손실을 키우는 장치가 된다",
        정책필드="max_entry_slip_pct", 최소=0, 최대=50,
    ),
    기준(
        "min_turnover_eok", "거래대금 최소 기준(억)", "숫자", "50",
        "최근 20일 평균 거래대금이 이보다 적으면 안 삽니다",
        "거래가 적은 종목은 매매할 때 체결가가 불리해진다(슬리피지). 우리는 슬리피지 "
        "실측이 0건이라 그 위험을 감당할 근거가 없다. 이 기준에 삼익THK(4억)와 "
        "국내 구리 ETF 전부가 걸려서 뺐다",
        최소=0, 최대=100000,
    ),
    기준(
        "sector_filter_enabled", "섹터 필터", "참거짓", "false",
    ...
    기준(
        "strategy", "매수 전략", "글자", "volume_surge_5d",
    ...
    기준(
        "sell_strategy", "매도 전략", "글자", "",
        "매도에만 따로 설정한 전략 키. 비어 있으면 매수 전략의 매도 규칙을 씁니다",
    ...
    # 구간 성적용 순위 지침 셋 (2026-09-04에 더함).
    #
    # **연 단위 셋과 자리를 따로 둔다.** 전에는 `rank_1st` 세 칸을 둘이
    # 나눠 썼는데, 거기 적힌 값(`worst_slice` 등)이 구간 성적 목록에 없는
    # 이름이라 구간 순위는 언제나 기본값으로 돌아갔다. 화면에서 1순위를
    # 바꿔도 전략 찾기 표와 쌓이는 순위에는 아무 영향이 없었다.
    #
    # 두 목록은 재는 단위가 달라서 겹치는 이름이 `max_drawdown` 하나뿐이다.
    # 한 자리를 나눠 쓸 수가 없다.
    기준(
        "rank_window_1st", "구간 성적 1순위 지침", "글자", "window_cagr",
        "보유 상한별 측정에서 전략을 견줄 때 제일 먼저 보는 것",
        "**이 칸은 주문을 내지 않는다. 무엇을 먼저 볼지만 정한다.** 화면의 "
        "전략 찾기 탭과 측정 결과를 쌓는 워크플로가 이 순서로 순위를 "
        "매긴다. 고를 수 있는 값은 `analysis/window_judgment.py`의 순위 "
        "지침 목록에 있다. 연 단위 지침(`rank_1st`)과 목록이 다르다",
        텔레그램가능=False,
    ),
    기준(
        "rank_window_2nd", "구간 성적 2순위 지침", "글자", "plus_ratio",
        "1순위가 비슷할 때 다음으로 보는 것",
        "지침마다 '이 정도 차이는 같다고 본다'는 폭이 있고, 그 폭 안이면 "
        "이 지침으로 가른다",
        텔레그램가능=False,
    ...
    기준(
        "rank_window_3rd", "구간 성적 3순위 지침", "글자", "p10",
        "1순위와 2순위가 다 비슷할 때 마지막으로 보는 것",
        "셋은 서로 달라야 한다. 성하지 않으면 기본값으로 돌아가고, 그 사실을 "
        "쌓기 로그에 적는다",
        텔레그램가능=False,
    ),
    기준(
        "sector_top_n", "선택 섹터 수", "정수", "3",
        "섹터 필터를 켰을 때 몇 섹터까지 고를 것인가",
        "섹터 거르기가 꺼져 있으면 화면 표시에만 쓰인다. 켜면 이 수만큼만 "
        "매수 대상이 된다",
        최소=1, 최대=10,
    ),
    기준(
        "min_sample_trades", "최소 표본 기준(거래 건수)", "정수", "5",
        "기간 검증·매일 검토에서 이 건수에 못 미치면 표본 부족으로 표시합니다",
        "**주문을 내지 않는다. 판단하기에 거래가 충분한지만 가른다.** "
        "2026-09-09까지는 20건으로 코드에 고정돼 있어서 화면에서 보이지도, "
        "바꾸지도 못했다. 낮추면 적은 거래로도 표본 부족 표시가 덜 뜨는 "
        "대신, 우연히 잘된 한두 건이 전체를 좌우했을 위험도 그만큼 커진다",
        최소=1, 최대=999, 텔레그램가능=False,
    ),
)
...
#: 시트에 있는 것이 맞지만 **파이썬이 안 쓰는** 이름.
#:
#: n8n 연결이 읽어 가는 값이다. `dashboard_key`는 화면 단추를 지키는
#: 값이고, n8n이 이 값과 비교해서 안 맞으면 거절한다.
#:
#: 위의 `기준들`에 넣지 않는 이유가 있다. 거기 넣으면 시트 초안과 화면과
#: 텔레그램과 문서에 이름과 기본값이 같이 실린다. 비밀값에는 그 자리가
#: 전부 새는 자리다.
#:
#: 여기에 안 적어 두면 `sync_sector_sheet.py --check`가 "모르는 이름"으로
#: 보고 **실행을 실패로 끝낸다.** 실제로 그랬다(2026-09-05).
바깥이름: frozenset[str] = frozenset({"dashboard_key"})

아는이름 = set(기준표) | set(바깥이름)
...
def parse_settings(설정: dict[str, str]) -> 시트설정:
    ...
    **네트워크 없이 시험할 수 있게 따로 뺐다.** 규칙이 이 함수의 전부이고,
    ...
    for 이름, 원값 in 설정.items():
        ...
        b = 기준표.get(키)
        if b is None:
            # n8n이 읽어 가는 이름은 파이썬이 안 쓸 뿐 잘못 적힌 것이 아니다.
            # 값도 여기에 담지 않는다. 담으면 로그와 화면으로 흘러 나간다.
            if 키 not in 바깥이름:
                모르는것.append(키)
            continue
        if 글자 == "":
