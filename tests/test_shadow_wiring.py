...
def _화면짝짓기(줄들: list[dict]) -> list[dict]:
    """`dashboard/app.js`의 `추적짝짓기`를 그대로 떼어 node로 실행한다.
    ...
    **같은 규칙이 두 곳에 있다.** 화면은 창구가 준 평평한 줄을 받아 짝을
    ...
    if not node:
        ...
        _pytest.skip("node가 없어 화면 쪽 규칙을 못 실행합니다")
...
#: CLAUDE.md의 화면 문구 지침이다. "잽니다"와 "재 봅니다"는 "계산합니다"와
#: "검증합니다"로, "돌려 보다"와 "돌리다"는 "계산하다"와 "실행하다"로 쓴다.
#: 2026-09-01에 그림자 추적 문구에서 같은 지적을 다시 받았다.
#:
#: 아래는 막으려는 말이다. 저장소 전체에서 걷어냈지만, 다시 들어오는 것을
#: 막으려고 목록은 그대로 둔다. 여기를 고칠 때는 반대로 뒤집히지 않았는지
#: 한 번 더 본다. 2026-09-01에 낱말을 일괄로 바꾸다 실제로 뒤집혔다.
