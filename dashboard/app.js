...
(() => {
  ...
  function 기준그리기(자료, 진짜) {
    ...
    const 모름 = !자료.전략;
    $("전략").innerHTML = (모름
      ? `<option value="" selected>(지금 무엇이 걸려 있는지 알 수 없습니다)</option>`
      : "") + 계열들.map((계열) => `
    ...
    $("전략알림").innerHTML = 못읽음
      ...
      : (읽기만
        ? '<div class="곁말">지금 걸려 있는 전략입니다. 여기서는 <b>보기만</b> 됩니다. '
          + '바꾸는 것은 돈이 나가는 결정이라 화면에서 조용히 되게 두지 않았습니다. '
          + '아래에서 다른 것을 골라도 규칙만 보여 주고 저장되지 않습니다.</div>'
        : "");
