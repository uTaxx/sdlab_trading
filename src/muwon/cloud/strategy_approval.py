...
**② 한 번 눌러서는 안 바뀐다.** 버튼을 누르면 `고름`이고, 확인 버튼까지
눌러야 `확정`이다. 폰 화면에서 손가락이 스치는 일이 실제로 일어난다.

**③ 지난 날짜 버튼은 안 듣는다.** 어제 온 메시지의 버튼이 그대로 살아
있다. 어제 판단으로 오늘 전략을 바꾸면 안 된다.

**④ 확정된 것만 반영한다.** `고름`에서 멈춘 것은 사람이 보다가 만 것이다.
그것을 반영하면 승인 단계가 없는 것과 같다.
...
def 고르기(
    ...
    session.add(줄)
    session.flush()
    return 예약결과(True, "선택했습니다. 확인 버튼을 눌러야 확정됩니다.", 줄)


def 확정하기(session: Session, 제안일: date, 새전략: str) -> 예약결과:
    """2단계. 확인 버튼까지 눌린 상태. 반영은 다음 거래일이다."""
    줄 = 지금예약(session)
    if 줄 is None:
        return 예약결과(False, "예약된 전략 변경이 없습니다. 새 목록에서 다시 눌러 주세요.")
    if 줄.제안일 != 제안일:
        # 규칙 ③. 어제 메시지의 버튼이 그대로 살아 있다.
        return 예약결과(
            False,
            f"{줄.제안일}에 낸 제안이 아닙니다. 오늘 온 목록에서 눌러 주세요.",
        )
    if 줄.새전략 != 새전략:
        return 예약결과(False, "선택한 전략과 확인한 전략이 다릅니다. 다시 골라 주세요.")
    if 줄.상태 == 확정:
        return 예약결과(True, "이미 확정되어 있습니다.", 줄)

    줄.상태 = 확정
    줄.바뀐때 = datetime.utcnow()  # noqa: DTZ003 — 기록용, tz 무관
    session.flush()
    return 예약결과(True, "확정했습니다. 다음 거래일 매수 후보 산출 전에 반영합니다.", 줄)
