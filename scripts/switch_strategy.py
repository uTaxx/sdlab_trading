...
def main() -> int:
    ...
    if 새매도:
        ...
        print(_성적(새매도))
        print()
        # 섞은 조합은 이름이 없으므로 성적표에도 없다. 그 사실을 말한다 —
        # 안 재 본 조합을 숫자 없이 내놓으면 재 본 것처럼 읽힌다.
        print("  ⚠ 매수와 매도를 섞은 이 조합 자체는 성적표에 없습니다.")
        print("    위 두 줄은 각 전략을 통째로 썼을 때의 숫자입니다.")
        print("    섞은 조합의 성적은 아직 재지 않았습니다.")
        나가는길 = _나가는길경고(키, 새매도)
        if 나가는길:
            print(f"  ⚠ {나가는길}")
    else:
