...
#: 최소 운용기간(기본 30일)을 안 따지는 경로 (2026-09-05에 나눔).
#:
#: ## 무엇을 막으려던 규칙인가
#:
#: 매일 저녁 검토가 "오늘은 이 전략이 1위입니다" 하고 후보를 낸다. 그것을
#: 매번 받아들이면 그때그때 성적 좋은 것으로 계속 갈아타게 되고, 그 방식은
#: 이미 계산해서 기각했다(설계안 §41, §36). **기계가 매일 조르는 것을
#: 막으려고 둔 값이다.**
#:
#: ## 그런데 주인까지 막고 있었다
#:
#: 화면에서 전략을 바꿀 수 있게 하고 나니(2026-09-04) 그 길도 같은 제한에
#: 걸렸다. 주인이 근거를 보고 직접 고른 것까지 30일 막는 것은 규칙의 뜻과
#: 다르다. 기계가 조르는 것이 아니기 때문이다.
#:
#: ## 어떻게 가르나
#:
#: 텔레그램 버튼은 **기계가 낸 후보를 받아들이는 것**이라 제한을 그대로
#: 받는다. 화면과 대화는 **목록에서 사람이 스스로 고르는 것**이라 안 받는다.
#: 누가 눌렀느냐가 아니라 무엇을 보고 골랐느냐로 가른다.
제한없는경로 = ("화면", "대화")
...
def 반영할것(
    ...
    **여기서 다시 검사한다.** 예약과 반영 사이에 밤이 하나 있어서 그 사이에
    ...
    if 최소운용일 > 0 and 줄.승인경로 not in 제한없는경로:
