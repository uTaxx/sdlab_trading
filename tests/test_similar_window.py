...
# ── 찾기()가 구간에서재기()에 인자를 그대로 넘긴다 ────────────


def test_찾기가_예수금_인자를_받는다():
    """scripts/run_strategy_review.py가 이 자리를 `예수금=` 키워드로 부른다.
    한 단계 안쪽 함수(`구간에서재기`)에만 이 인자가 있고 `찾기`에는 없어서
    2026-09-08에 매일 저녁 검토가 이 자리에서 조용히 실패하고 있었다.
    `except Exception`으로 삼켜져서 워크플로는 계속 초록불이었다."""
    from muwon.strategy.registry import build_strategy

    시세 = _시세(날수=400)
    끝 = max(시세["000000"]["trade_date"])
    결과 = 이단계.찾기(
        시세, {"volume_surge_3d": (lambda: build_strategy("volume_surge_3d"))},
        이단계.RiskPolicy(max_holding_days=5), 기준일=끝, 예수금=10_000_000.0,
    )
    assert 결과.기준일 is not None
