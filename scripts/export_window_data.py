...
칸들 = (
    # **종목수는 줄마다 다르다.** 조건마다 다른 날 재고, 그 사이에 매매 대상
    # 목록이 바뀔 수 있다. 2026-09-06에 실제로 그랬다. 종목을 63개에서 39개로
    # 좁히고 최근 3개월만 다시 쟀는데, 5년 줄은 63종목 때 값 그대로인 채로
    # 파일 머리에는 39종목이라고 적혀 있었다. 화면은 그 머리를 읽는다.
    "전략", "상한", "슬리피지", "시작일", "국면", "종목수",
...
def 한줄(ㄱ) -> list:
    ...
    값들 = {
        ...
        "시작일": ㄱ.시작일.isoformat(), "국면": ㄱ.국면, "종목수": ㄱ.종목수,
    ...
    **잰 조건 목록을 따로 담는다.** 화면이 "이 조건은 아직 계산하지
    ...
    with session_factory() as 세션:
        for 열쇠, _ in 매매대상들:
            ...
            # 머리에는 가장 최근에 잰 값을 적되, **줄마다 제 값을 따로
            # 싣는다.** 머리 하나로 전부를 말하면 옛 조건이 새 종목 수로
            # 둔갑한다(2026-09-06에 실제로 그랬다).
            최근 = max(잰것들, key=lambda ㄱ: ㄱ.잰날)
            종목수들 = sorted({ㄱ.종목수 for ㄱ in 잰것들 if ㄱ.종목수})
            묶음[열쇠] = {
                "잰날": 최근.잰날.isoformat(),
                "처음잰날": 잰날들[0].isoformat(),
                "종목수": 최근.종목수,
                # 섞여 있으면 화면이 그 사실을 적어야 한다. 하나뿐이면
                # 섞이지 않은 것이다.
                "종목수들": 종목수들,
                "끝일": 최근.끝일.isoformat(),
