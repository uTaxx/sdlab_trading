"""지금 설정된 전략을 최근 3개월·12개월·5년에 실행해 보고 시트에 남긴다.
...
과거 시세로 실행해 보기만 한다. 증권사에 붙지 않고, 상태 DB도 고치지 않는다.
...
기간검증머리 = [
    ...
    "수익률%", "최대낙폭%", "최악구간", "최악구간%", "승률%", "손익비",
    "거래수", "표본충분", "기준", "상태", "까닭", "평가", "신호등급", "신호",
]

탭이름 = "기간검증"

#: 비교 모드가 쓰는 탭. 검증과 섞지 않는다. 저기는 "지금 설정된 전략이
...
def 전략고르기(글: str) -> tuple[list[str], list[str], bool]:
    ...
    **모르는 열쇠말은 조용히 버리지 않고 여기서 멈춘다.** 하나를 잘못 적었는데
    ...
    # 파는 쪽을 비교할 때는 사는 쪽이 하나여야 한다. 둘 다 실행하면 표의 한 줄이
    # 무엇을 바꾼 결과인지 알 수 없다.
    ...
    if 파는쪽전부비교:
        ...
        if len(사는키) != 1 or 사는키[0] in ("전부", "all"):
            raise ValueError(
                ...
                "둘 다 실행하면 표의 한 줄이 무엇을 바꾼 결과인지 알 수 "
                f"없습니다: {글!r}"
            )
...
def 진짜로(골라진것, 잰때: datetime, 인자, sheet_id: str) -> int:
    ...
    if not 성적들:
        raise RuntimeError(f"시세가 모자라 한 구간도 못 실행했습니다: {', '.join(못돌린것)}")

    화면에(성적들, 사는키, 파는키, 기준, {ㄱ.이름: ㄱ for ㄱ in 골라진것})
    if 못돌린것:
        print(f"못 실행한 구간: {', '.join(못돌린것)} (시세가 모자랍니다)")

    # 전략을 다시 볼 이유가 있나. **바꾸라는 말이 아니라 보라는 말이다.**
    성적모음 = {ㅅ.이름: ㅅ for ㅅ in 성적들}
    난것 = 전략신호(성적모음)
    등급, 신호 = 신호글(난것)
    print(f"■ 전략 신호: {등급}")
    print(신호 + "\n")
    ...
    if 등급 != "이상없음":
        알리기(알림글(등급, 난것, 전략글(사는키, 파는키), 성적모음))
    return 0
