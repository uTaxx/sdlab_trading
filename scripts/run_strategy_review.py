...
비슷한시점탭 = "비슷한시점"
비슷한시점머리 = ["열쇠", "잰때", "기준일", "번호", "시작", "끝"]


def 비슷한시점시트줄(찾은것, 끝, 잰때) -> list[list[str]]:
    """`찾은것.구간들`(실제로 비슷했다고 찾은 과거 시점들)을 시트 줄로.
    순수 함수다.

    **화면이 "언제였나"를 보여 줄 수 있어야 한다.** 전략별 성적표만 있으면
    비슷했던 과거가 몇 번, 어느 시점인지 사람이 알 방법이 없다. 번호는
    거리(가까운 정도) 순서를 그대로 쓴다. 1번이 가장 비슷했던 시점이다.

    구간이 하나도 없으면 빈 목록을 낸다. 상태와 사유는 `비슷한구간` 탭이
    이미 담고 있다."""
    if not 찾은것.구간들:
        return []
    return [
        [f"T{끝}|{i}", f"{잰때:%Y-%m-%d %H:%M}", f"{찾은것.기준일}",
         str(i), f"{ㄱ.시작}", f"{ㄱ.끝}"]
        for i, ㄱ in enumerate(찾은것.구간들, 1)
    ]


def 비슷한구간남기기(sheet_id: str, 긴시세, 정책, 끝) -> str:
    """전략을 고르는 2단계. 최근 20거래일과 비슷했던 과거를 찾고, 그때
    어느 전략이 좋았는지 시트 `비슷한구간` 탭에, 그 과거가 정확히 언제인지
    시트 `비슷한시점` 탭에 남긴다.
    ...
    **실패해도 삼킨다.** 이 계산이 막혀도 3~4단계(구간별 순위, 예약)는
    ...
    try:
        ...
        시점줄들 = 비슷한시점시트줄(찾은것, 끝, 잰때)
        if 시점줄들:
            시점올린수 = append(sheet_id, 비슷한시점탭, 비슷한시점머리, 시점줄들)
            print(f"시트 '{비슷한시점탭}'에 {시점올린수}줄 올렸습니다.", file=sys.stderr)
        return 표본글
