...
def 전략신호(성적들: dict, 순위: dict | None = None) -> list[신호]:
    ...
    **아무 이유도 못 찾으면 빈 목록을 돌려준다.** 억지로 한 줄 만들면
    ...
    if 긴것 and 한해 and 긴것.metrics.max_drawdown_pct < -1:
        ...
        if 몫 >= 0.9:
            난것.append(신호(
                "살펴볼것", "5년 중 제일 나빴던 구간이 최근 1년 안에 있습니다",
                f"최근 12개월 낙폭이 {한해.metrics.max_drawdown_pct:.2f}%인데 "
                f"5년 낙폭이 {긴것.metrics.max_drawdown_pct:.2f}%입니다. "
                "제일 아팠던 구간을 지금 지나고 있다는 뜻입니다.",
            ))
...
def 신호글(난것: list[신호]) -> tuple[str, str]:
    ...
    for ㅅ in 난것:
        등급 = 더센등급(등급, ㅅ.등급)
        줄들.append(f"[{ㅅ.등급}] {ㅅ.제목} — {ㅅ.설명}")
    줄들.append(
        "전략을 바꾸는 것은 돈이 나가는 결정이라 화면에서는 안 됩니다. "
        "바꾸기 전에 성적표와 같은 자로 다시 재는 것이 순서입니다."
    )
    return 등급, "\n".join(줄들)
