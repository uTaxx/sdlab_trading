...
내부 이름이 새는 문제다. 8월 26일까지는 줄표(—) 금지 검사도 여기 있었는데,
그건 취향이지 결함이 아니어서 뺐다. 문장을 어떻게 쓸지는 CLAUDE.md에서도
규칙으로 두지 않는다.
...
def test_텔레그램에_안_먹는_기호가_없다(이름, 만들기):
    ...
    assert not 있는것, (
        ...
        "텔레그램은 parse_mode 없이 보내므로 기호가 그대로 찍힙니다 — 말로 쓰세요."
    )
...
def test_영어_코드_이름이_사용자에게_안_나간다(이름, 만들기):
    ...
    글 = 만들기()
    # 주소 줄은 뺀다. https://utaxx.github.io/sdlab_trading/ 의 밑줄은
    # 내부 이름이 샌 것이 아니라 저장소 이름 그대로다.
    본문 = "\n".join(ㄹ for ㄹ in 글.splitlines() if "http" not in ㄹ)

    assert "volume_surge" not in 글
    assert "_" not in 본문.replace("─", ""), "밑줄이 들어간 내부 이름이 샜을 수 있습니다"
