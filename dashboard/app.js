/* ─────────────────────────────────────────────────────────────
   ...
   보게 하려는 것이다 — 여럿으로 나누면 하나를 빠뜨리고 빠뜨린 줄도 모른다.
     ...
     POST {바탕}/webhook/sdlab   { 열쇠, 무엇, ...인자 }
       ...
       무엇: "기준"      →  { 매매켜짐, 매도켜짐, 전략, 매도전략, 손절, 익절, 비중, 동시보유 }
       무엇: "기준저장"  →  { 바꿀것: {...} }  →  { 된것: true }
...
(() => {
  ...
  function 기준그리기(자료, 진짜) {
    ...
    $("익절곁말").textContent = 익절 > 0
      ? `산 값에서 ${익절}% 오르면 팝니다. 손절 다음, 보유 기간과 매도 신호보다 먼저 걸립니다.`
      : "지금은 꺼져 있습니다. 켜 봤더니 도움이 안 돼서 뺐습니다 — 이긴 매매 599건의 되돌림 중앙값이 +0.28%라 익절로 건질 것이 없었습니다.";

    // 전략 드롭다운은 잠그지 않는다. 목록과 규칙은 정적 자료라 창구가
    // 없어도 진짜고, 골라 보는 것만으로는 아무것도 안 바뀐다.
    ["킬전환", "매도전환", "손절", "비중", "동시보유", "익절", "기준저장"].forEach((id) => {
      $(id).disabled = !진짜;
    });
  }
  ...
  $("전략").addEventListener("change", () => { 전략설명갱신(); 매도설명갱신(); });
  $("매도전략").addEventListener("change", 매도설명갱신);
  ...
  $("기준저장").addEventListener("click", async () => {
    ...
    // 익절만 0을 받는다. 여기서 0은 빈 값이 아니라 "끈다"는 뜻이다.
    // 나머지 칸에 0을 넣으면 매매가 성립하지 않는다.
    const 영도되는칸 = ["익절"];
