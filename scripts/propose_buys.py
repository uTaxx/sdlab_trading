...
def 게이트진단글(껍데기, 종목표: dict, 이름표: dict) -> str:
    """"거래량 급증이 없는 건가, 미국 섹터가 문제인 건가"에 답한다.

    지금 설정된 전략(volume_surge_3d_us60_2)처럼 원래 전략 위에 미국 섹터
    조건을 하나 더 얹은 전략만 이 진단을 남긴다(`USSectorGateStrategy`).
    그 전략이 아니면 조용히 빈 글을 돌려준다."""
    게이트 = getattr(껍데기, "마지막게이트", None)
    if 게이트 is None or 게이트.통과매수수 > 0:
        return ""

    if 게이트.원래전략매수수 == 0:
        return ("거래량 급증 조건 자체가 오늘 어떤 종목에서도 나지 않았습니다. "
                "미국 섹터 조건은 이번 판단에 영향을 주지 않았습니다.")

    막힌이름 = ", ".join(
        f"{종목표[심][1].name}({심})" if 심 in 종목표 else 심
        for 심, _ in 게이트.막힌것
    )
    강한이름 = [이름표.get(코드, 코드) for 코드 in sorted(게이트.강한섹터)]
    강한글 = (f"오늘 미국 섹터 조건을 통과한 섹터는 {', '.join(강한이름)}입니다."
             if 강한이름 else "오늘은 미국 섹터 조건을 통과한 섹터가 없습니다.")
    return (f"거래량 급증 조건은 {게이트.원래전략매수수}종목에서 났습니다"
            f"({막힌이름}). 다만 미국 섹터 조건에 안 걸려 전부 제외됐습니다. "
            f"{강한글}")
