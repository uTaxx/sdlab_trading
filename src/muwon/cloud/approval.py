...
def _전략조건글(전략키들: Sequence[str]) -> list[str]:
    """지금 전략이 실제로 무엇을 찾는지 한 줄로.

    **후보가 없는 날 "없습니다"만 보내면 무엇을 기준으로 없었는지 알 수
    없다.** 전략 카탈로그에 이미 있는 쉬운설명(전문용어 없이 쓴 글)을
    그대로 쓴다. 여기서 새로 문장을 짓지 않는다."""
    if not 전략키들:
        return []
    try:
        from muwon.strategy.registry import get_definition
    except Exception:  # noqa: BLE001 (설명을 못 찾는다고 알림이 죽으면 안 된다)
        return []
    ...
    for 키 in 전략키들:
        try:
            정의 = get_definition(키)
        except Exception:  # noqa: BLE001, S112 (모르는 키라고 알림이 죽으면 안 된다)
            continue
        if not 정의.쉬운설명:
            continue
        줄들.append(f"  {정의.화면이름}: {정의.쉬운설명}")
    if not 줄들:
        return []
    return ["지금 전략이 찾는 종목", *줄들]


def _시세못본글(못본것: Sequence[str]) -> list[str]:
    """시세를 못 받아 아예 살펴보지 못한 종목.

    **살펴본 종목 수에도 들어 있지 않다.** 조용히 빠지면 "신호가 없어서
    0개"와 "일부는 애초에 못 봐서 0개"를 구별할 수 없다."""
    if not 못본것:
        return []
    줄들 = ["시세를 못 받아 살펴보지 못한 종목 " + f"{len(못본것)}개"]
    보일것 = list(못본것)[:10]
    for ㄱ in 보일것:
        줄들.append(f"  · {ㄱ}")
    남은 = len(못본것) - len(보일것)
    if 남은 > 0:
        줄들.append(f"  · 외 {남은}개 더")
    줄들.append("  살펴본 종목 수에는 들어 있지 않습니다")
    return 줄들
...
def 알림글(후보들, 날짜: date, 주소: str, 살펴본수: int | None = None,
         ...
         전략변경: str = "", 상한초과=(), 전략키들: Sequence[str] = (),
         못본것: Sequence[str] = (), 게이트설명: str = "") -> str:
    ...
    `전략키들`은 지금 전략이 실제로 무엇을 찾는지(쉬운설명)를 후보 없는 날에
    붙이고, `못본것`은 시세를 못 받아 살펴본 수에도 안 들어간 종목을 적는다.
    `게이트설명`은 미국 섹터처럼 원래 전략 위에 조건을 하나 더 얹은 전략에서
    "원래 조건 자체가 안 났다"와 "원래 조건은 났는데 위에 얹은 조건에 막혔다"를
    가른다. 셋 다 없으면 그 자리를 그냥 뺀다."""
    머리 = f"📋 {_날짜글(날짜)}"
    본것 = []
    if 살펴본수 is not None:
